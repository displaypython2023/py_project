...
class App(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # Change button
        submit_button = ttk.Button(self, text='Change',command=self.check_pass)
        submit_button.grid(column=0, row=3, **padding)

    def check_pass(self):
        if self.password_ok == True :
            tk.messagebox.showinfo("showinfo", "password changed OK")
        else:
            tk.messagebox.showerror("showerror", "password not the same!!!")
